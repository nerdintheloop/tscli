...
import { InternalCommand, USAGE_INFORMATION_FLAG_LONG, USAGE_INFORMATION_FLAG_SHORT } from './commands.ts'
...
export const composeCommandUsageInformation = (name: string, run: Run) => {
  ...
  // Header
  if (alias) {
    cliCommand = bold(alias)

    if (alias !== name) cliCommand += ` ${bold(name)}`
  } else {
    const runtime = getRuntime()

    cliCommand = cliScriptPath

    if (runtime === Runtime.Deno) cliCommand = `${dim('[...permissions]')} ${cliCommand}`

    cliCommand = `${bold(runtime)} ${cliCommand}`
  }

  cliCommand += ` [...flags]`

  if (positionals) {
    for (const name of positionals.keys()) cliCommand += ` ${name}`
  }

  if (rest) cliCommand += ` ...${rest.name}`

  let output = `${bold('Usage:')} ${cliCommand}`
...
export const composeMainUsageInformation = (run: Run) => {
  ...
  if (alias) cliCommand = `${bold(alias)}`
  else {
    cliCommand = cliScriptPath
  ...
  // Header
  let output = `${bold('Usage:')} ${cliCommand} ${bold('<command>')} [...args] ${dim(italic(`[ +[.] <command> [...args] ... ]`))}${EOL.repeat(2)}`
  ...
  output += `Print command usage information: ${cliCommand} ${bold('<command>')} --help/-h
${alias ? 'Update' : 'Create'} shell aliases scripts:    ${cliCommand} ${bold(InternalCommand.CreateShellAliases)}
Learn more about ${bold('tscli')}:          https://nerdintheloop.pages.dev/${bold('tscli')}${EOL}`
