...
import { getCommandName, InternalCommand, runCLICommand, type CLI } from './commands.ts'
  ...
  for (const [index, error] of errors.entries()) {
    if (index > 0) {
      ...
      if (error instanceof Error) {
        ...
        if (!stack) {
          const cause = errors[index - 1] as CLICommandError
          const causeCLICommandName = getCommandName(cause.command)
          const causeCLICommandModulePath = run.commands[causeCLICommandName].modulePath!

          location = `in ${cyan(`.${causeCLICommandModulePath.replace(cliDirectoryPath!, '')}`)}`
        }
...
const executeRunCommand = async (command: string, run: Run): Promise<number | void> => {
  if (command.startsWith(InternalCommand.CreateShellAliases)) await createShellAliasesScripts(run)
  else {
    const output = await runCLICommand(command, run!)
...
export const executeRunCommands = async (run: Run) => {
  ...
  const validCommandNames = [...Object.keys(run.commands), InternalCommand.CreateShellAliases]
